...
服务器端：
    是一个纯后端提供接口服务的SpringBoot项目，已有打包文件，打包位置为：Chat-Mini-Programs/聊天程序服务器端/out/artifacts/_war_exploded。
...
Server-side: is a pure back-end interface services SpringBoot project, there are packaging files, packaging location: Chat-Mini-Programs/聊天程序服务器端/out/artifacts/_war_exploded. Because I deleted the information about the server-side database connection, you needed to add the database connection information yourself when deploying the server-side project. And I've added the SQL files needed for the database to the server-side project.
